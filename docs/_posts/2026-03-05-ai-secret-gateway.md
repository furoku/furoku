...
2026年3月、Claude Code を使った運用中に Google 広告アカウント（MCC）が乗っ取られ、被害額8桁後半という事例が報告された（[出典: @hassii_ad on X](https://x.com/hassii_ad/status/2028399491565633731)）。

その後、原因が特定された。流れはこうだ：

1. **Claude Code が `.env` を読み込んだ** — 読み込み禁止の設定になっていたにもかかわらず、無視された
2. **認証情報がログに出力された** — 読み込まれた値がそのままログに残った
3. **漏洩・悪用された** — 深夜に MCC が正規権限で操作され、気づいたときには手遅れだった

MCC（管理者アカウント）が乗っ取られると、配下の大量の広告アカウントを使って犯人が他人のお金で広告を配信できる。媒体側からは通常の操作と見分けがつかず、補償は絶望的だ。

**「読み込み禁止にしていたのに無視された」** — これがまさに、「お願いベースの防御」の限界を示している。
